...
def get_eu_privacy_mode(uid: str) -> bool:
    """True if the user has EU Privacy Mode enabled.

    Persisted at `users/{uid}.eu_privacy_mode`. Defaults to False so existing
    users see no behavior change. Read fresh on every request — privacy
    settings cannot be cached without creating a data-spillage window if a
    toggle-OFF→ON fails to invalidate.
    """
    user_ref = db.collection('users').document(uid)
    data = user_ref.get().to_dict() or {}
    return bool(data.get('eu_privacy_mode', False))


def set_eu_privacy_mode(uid: str, enabled: bool) -> None:
    """Persist the EU Privacy Mode toggle for `uid`."""
    user_ref = db.collection('users').document(uid)
    user_ref.set({'eu_privacy_mode': bool(enabled)}, merge=True)
