...
# Optional providers users may enroll on top of the required four (e.g. Regolo
# for EU Privacy Mode). Not part of the BYOK-free-plan gate, but accepted by
# the activate endpoint so the client can pin a fingerprint.
_BYOK_OPTIONAL_PROVIDERS = {'regolo'}
_BYOK_ALLOWED_PROVIDERS = _BYOK_REQUIRED_PROVIDERS | _BYOK_OPTIONAL_PROVIDERS


class EUPrivacyModeRequest(BaseModel):
    enabled: bool


@router.post('/v1/users/me/eu-privacy-mode', tags=['v1'])
def set_eu_privacy_mode_endpoint(
    data: EUPrivacyModeRequest,
    uid: str = Depends(auth.get_current_user_uid),
):
    """Toggle EU Privacy Mode for the authenticated user.

    When enabled, supported LLM features re-route to Regolo's EU-hosted
    inference; embedding/vision/web-search/chat-agent features are hard-
    blocked (NOT silently fallen back) so no user data leaves EU
    infrastructure. The flag is persisted to Firestore and read fresh on
    every request.
    """
    users_db.set_eu_privacy_mode(uid, data.enabled)
    return {'eu_privacy_mode': data.enabled}


@router.get('/v1/users/me/eu-privacy-mode', tags=['v1'])
def get_eu_privacy_mode_endpoint(uid: str = Depends(auth.get_current_user_uid)):
    """Return the current EU Privacy Mode flag for the authenticated user."""
    return {'eu_privacy_mode': users_db.get_eu_privacy_mode(uid)}
