...
# ENCRYPTION_SECRET must be exported before running. The upstream omi/backend/test.sh hardcodes a dev value;
# we redact it here so the integration repo passes secret-scanning. Restore the upstream value when applying the patch.
: "${ENCRYPTION_SECRET:?must be set — see upstream omi/backend/test.sh for the dev fixture value}"
...
pytest tests/unit/test_regolo_provider.py -v
