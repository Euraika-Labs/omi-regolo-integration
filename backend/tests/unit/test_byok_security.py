...
# ENCRYPTION_SECRET is set by test.sh in CI; locally export it before pytest.
assert os.environ.get('ENCRYPTION_SECRET'), 'ENCRYPTION_SECRET must be set; see upstream omi/backend/test.sh'
...
class TestBYOKHeadersConstant:
    def test_headers_has_all_known_providers(self):
        from utils.byok import BYOK_HEADERS

        assert set(BYOK_HEADERS.keys()) == {'openai', 'anthropic', 'gemini', 'deepgram', 'regolo'}
